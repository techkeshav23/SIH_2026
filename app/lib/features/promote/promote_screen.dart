...
class _BoostScreenState extends ConsumerState<BoostScreen> {
  ...
  Future<void> _boost(bool hi) async {
    setState(() => _busy = true);
    // TODO(friend): POST /promote/boost { product_id, budget_rupees, days, audience, image_source }
    //               -> Meta Marketing API: Campaign -> AdSet -> Creative -> Ad.
    await Future.delayed(const Duration(milliseconds: 1400)); // mock
    if (!mounted) return;
